...
| Workbook structure | seven sheets, annual columns — see `WORKBOOK_FORMAT.md` | Learned from a reference CFO model. Conclusion first (Summary), then drivers, then the audit trail (Sources & Notes). Sixty monthly columns is not a document anyone reads. |
| Colour is load-bearing | blue input · green cross-sheet link · black computed here · yellow fill = tool-supplied | The standard corporate-finance convention. The green/black split is the part most models omit and the part that makes a workbook auditable: it says whether a number was computed *here* or arrived from elsewhere. Benchmarks render yellow, so the lines the company did not give you are the visible ones. |
| Costs negative, subtotals add | `Gross profit = Revenue + COGS` | Lets a reader check a column by summing it, and makes a sign error visible instead of silent. |
| Annual restatement, stated as such | exact to EBITDA, ±5% on cash | Revenue, gross profit and EBITDA are definitionally aggregable and match to the dollar. Working capital and loss carryforward are path-dependent within a year, so they differ modestly. The Summary says so rather than implying parity, and the tolerance is pinned in the tests. |
...
| Results emailed via Resend | `notify.py`, called from `pipeline.run` | Both the CLI and the web app get it from one implementation. Sent last, only after both artifacts exist, and a delivery failure is reported rather than raised — losing a built model to a mail outage would be absurd. |
| The page's email disclosure is derived, not written | `Settings.email_enabled` | Someone uploading a confidential deck is entitled to know it will be forwarded. Deriving the sentence from configuration means the page cannot claim an email that is not sent, or stay silent about one that is. |
